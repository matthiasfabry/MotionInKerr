...
eps here is \\dot{\\Mathcal{E}} in the paper.
"""
...
data = np.array([[np.nan, -0.99,  -0.9,  -0.5,   0.0,   0.2,   0.5,    0.8,    0.9,   0.99,  0.999],
...
def eps_at_isco(a):
    """
    Linearly interpolates the data of Finn and Thorne to get the relativistic correction factor at the ISCO
    for a given spin
    :param a: float, spin of the BH
    :return: float, relativistic correction factor
    """
    if a > 0.999:
        raise ValueError('Spin too high for Finn&Thornes corrections')
    i = 1
    while a >= data[0][i]:
        if a == data[0][i]:
            return data[1][i]
...
def eps(r, a):
    """
    linearly interpolates the data of Finn and Thorne to get the relativistic correction factor
     for a given radius and spin
    :param r: float, BL radius of the particle
    :param a: float, spin of the BH
    :return: float, relativistic correction factor
    """
    if a > 0.999:
        raise ValueError('Spin too high for Finn&Thornes corrections')
    if r > 1.05:
        raise ValueError('Radius too high for represented data from FT')
    i, j = 1, 1
    while a >= data[0][i]:
        i += 1
    while r >= data[j][0]:
    ...
    # print(points, values)
    fit = itp.LinearNDInterpolator(points, values)
    return fit(r, a)
